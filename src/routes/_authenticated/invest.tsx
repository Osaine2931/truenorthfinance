...
import { Sparkles, TrendingUp, Lock, Calculator, Layers } from "lucide-react";
import {
  usePlans,
  useWallet,
  useCreateInvestment,
  ...
  async function confirm() {
    ...
    if (amount > balance) {
      toast.error("Insufficient deposited balance", {
        description: "The welcome bonus cannot be used to purchase investments.",
      });
      return;
    }
    try {
      await createInvestment.mutateAsync({ plan: selected, amount });
      toast.success(`Invested ${formatCurrency(amount)} in ${selected.name}`);
      setSelected(null);
    } catch (err) {
    ...
    <div className="animate-fade-up space-y-6">
      ...
      {selected && (
        <div className="fixed inset-0 z-50 grid place-items-end bg-navy/40 backdrop-blur-sm sm:place-items-center">
          <div className="w-full max-w-md rounded-t-3xl bg-card p-6 shadow-elevated sm:rounded-2xl">
            ...
            <Field label="Amount (USD)" hint={`Available to invest: ${formatCurrency(balance)}`}>
              <input
                type="number"
                value={amount}
            ...
            <div className="mt-4 rounded-2xl bg-royal-soft p-4">
              <p className="text-xs text-muted-foreground">Projected profit at maturity</p>
              <p className="font-display text-2xl font-semibold text-royal">{formatCurrency(expected)}</p>
            ...
            <div className="mt-5 flex gap-2">
              ...
              <button onClick={confirm} disabled={createInvestment.isPending} className={`${btnPrimary} flex-1`}>
                Confirm
              </button>
            </div>
          </div>
