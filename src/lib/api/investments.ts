  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (amount < minimumRequired)
        throw new Error(`Minimum investment is ${formatCurrency(minimumRequired, 0)}`);
      ...
      if (availableBalance < amount) {
        throw new Error(
          `Your available balance is ${formatCurrency(availableBalance, 0)}. Add funds to cover ${formatCurrency(amount, 0)} before investing.`,
        );
      }
...
export async function updateInvestmentProgress() {
  ...
  for (const investment of investments ?? []) {
    ...
    if (ended) {
      await supabase
        .from("investments")
        .update({ status: "completed", profit_earned: investment.expected_profit })
        .eq("id", investment.id);
      await supabase.from("notifications").insert({
